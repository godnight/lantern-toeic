...
export default function ThemeLibrary({selected,onApply,onBack}:{selected:Theme;onApply:(id:string)=>void;onBack:()=>void}){
 ...
 return <section className="art-library">
  ...
  {theme.download&&<div className="art-download-row"><a className="button primary" href={theme.download} download><Download size={16}/>下载原创素材包</a><p>桌面横图＋手机竖图，含来源和使用说明。</p></div>}
  <div className="section-heading"><h2>应用素材</h2><span>{theme.assets.length} 幅 · 原创生成</span></div>
  {theme.assets.length?<div className="art-asset-grid">{theme.assets.map(asset=><article className="art-asset" key={asset.id}><div className={'art-asset-image '+(asset.height>asset.width?'portrait':'')}><img src={asset.path} alt={asset.title} width={asset.width} height={asset.height} loading="lazy"/></div><div className="art-asset-copy"><h3>{asset.title}</h3><p>{asset.width} × {asset.height} · AI 生成</p><button onClick={()=>setPreview(asset)} className="text-button">打开原图<ArrowUpRight size={15}/></button></div></article>)}</div>:<p className="art-note">这套主题以纯色和排版呈现，无背景插画。</p>}
  <Dialog open={!!preview} onOpenChange={open=>{if(!open)setPreview(null);}}><DialogContent style={{maxWidth:'min(1100px, calc(100vw - 24px))'}}><DialogHeader><DialogTitle>{preview?.title||'素材预览'}</DialogTitle><DialogDescription>{preview?`${preview.width} × ${preview.height} · 原始素材`:'查看主题原图'}</DialogDescription></DialogHeader>{preview&&<img src={preview.path} alt={preview.title} width={preview.width} height={preview.height} style={{display:'block',width:'100%',maxHeight:'70dvh',objectFit:'contain'}}/>}</DialogContent></Dialog>
  {id!=='paper'&&<><div className="section-heading art-reference-heading"><h2>灵感参考库</h2><span>原作入口</span></div><p className="art-note">按角色、场景与 Boss 整理。点开查看官方或作者原作；参考图不随应用打包。</p><div className="art-download-row"><a className="button secondary" target="_blank" rel="noreferrer" href={id==='hollow'?'https://drive.google.com/drive/folders/1SPCRaalJJepKYOQ4fdxZEzqNukJiUQCo':'https://drive.google.com/drive/folders/1VE_YoUA36IwZRHnZhvEGUhqgU2GIEm4y'}>打开官方素材包<ArrowUpRight size={16}/></a><p>由游戏官网提供；使用范围以原站说明为准。</p></div><Tabs value={category} onValueChange={setCategory}><TabsList className="resource-tabs">{[{id:'all',label:'全部'},{id:'character',label:'角色'},{id:'scene',label:'场景'},{id:'boss',label:'Boss'}].map(c=><TabsTrigger value={c.id} key={c.id}>{c.label}</TabsTrigger>)}</TabsList></Tabs><div className="art-reference-grid">{references.map(r=><article key={r.id} className="art-reference"><span className="eyebrow">{r.category.toUpperCase()}</span><h3>{r.title}</h3><p>{r.author}{r.width&&r.height?` · ${r.width} × ${r.height}`:''}</p><div><a href={r.sourceUrl} target="_blank" rel="noreferrer" className="text-button">查看原作<ArrowUpRight size={15}/></a>{r.imageUrl&&<a href={r.imageUrl} target="_blank" rel="noreferrer" className="text-button">原图入口<ArrowUpRight size={15}/></a>}</div><small>{r.rightsStatus==='pending'?'二创 · 待确认使用范围':'官方参考 · 仅外链'} · 核验 {r.checkedAt}</small></article>)}</div>{references.length===0&&<p className="art-note">这个分类暂未收录，切换其他分类查看。</p>}</>}
 </section>;
}
