...
- 原生返回键先查询 Web 历史，无历史时交给系统。React 页签和弹窗目前不是浏览器历史条目，尚未增加关闭弹窗的原生桥接。
...
- 进入后台、锁屏和来电对录音的影响。此容器尚无原生生命周期录音桥接，不能仅凭网页的可见性逻辑宣布通过。
- 发布前必须补齐：系统返回无 Web 历史时可能绕过 React 的录音关闭保护；组件的 `active` 状态目前也不代表应用前台状态。需接入原生前后台事件和录音停止/保存逻辑，再验证权限弹窗等待期间切后台的行为。
