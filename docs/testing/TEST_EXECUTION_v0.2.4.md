# v0.2.4 鸿蒙访问与账号边界测试执行报告

## 后续：删除Android/iOS与交接修正（2026-09-13）

用户要求从源码树删除两平台，已删除工程、工作流、Capacitor配置/依赖，更新既有平台范围回归并保留鸿蒙共享前端。类型检查、35/35行为回归、鸿蒙共享构建和23项资源逐字节核对通过；初始化脚本语法检查通过。工作流与源码发布初始化只安装根目录依赖，干净CI结果以PR #18最新提交为准。

认证readiness路径按官方SDK文档改为AppScope/resources/rawfile，配置仍缺失。报告模式显示未就绪，强制模式返回2；不发送短信、不创建登录态。HAP编译签名和真机验收仍未完成。下面保留的是删除前v0.2.4的构建与验收记录，不把历史结果写成所有后续修改都已执行。

执行日期：2026-09-13（UTC）。范围为鸿蒙安装说明、Web/PWA与鸿蒙原生账号边界、平台收敛、版本同步及相关生产构建。总设计见 [40个历史跨端风险场景](TEST_DESIGN.md)。本报告只把实际执行的层级记为通过，不把共享资源构建写成HAP或真机验收。

## 自动化与构建

| 检查 | 结果 | 能证明与边界 |
|---|---|---|
| `npm run typecheck` | 通过 | 当前TypeScript可编译；不等于浏览器或SDK运行 |
| `npm run validate:content` | 通过：68道原创题、5类口语、22条官方资料 | 结构完整；不等于教师审校或复用许可 |
| `npm run validate:art` | 通过：3主题、8幅内置原创图、19个外部参考 | 目录、声明与哈希一致；不等于逐项视觉/许可复核 |
| `npm run test:study` | 通过：35/35 | 新增P07与平台范围源码行为覆盖：Web/PWA同账号说明、鸿蒙本机说明、安全在线外链、归档CI及不收集手机号 |
| Sites生产构建脚本 | 通过 | Vinext五阶段生产构建完成；`/`、`/api/study`、`/api/recordings`路由生成 |
| `node --test tests/*.test.mjs` | 通过：5/5 | 生产学习壳、安装元数据、CSS/语义构建后检查通过 |
| `npm --prefix mobile run build` | 通过 | 历史命名的鸿蒙共享静态前端生成；约669 kB主JS触发既有分包警告，不代表任何原生安装包 |
| Harmony `sync:web` + `check:web` | 通过：23个共享文件一致 | 只证明ArkWeb内置网页资源同步，不是HAP编译 |
| Harmony工具链报告 | 阻塞 | 找到仓库Hvigor启动脚本；缺OHPM、HDC和SDK目录，`compiled/signed/deviceTested`均为false |
| `npm --prefix harmony run check:auth` | 未就绪（按预期保持关闭） | AGC配置、认证SDK声明和HTTPS认证API地址均缺；只报告前置，不发送短信或创建登录态 |
| `npm run lint` | 未通过：256错误、2290警告 | 仓库全量Lint包含既有源码、CommonJS测试与生成资源问题；本次正式门禁以类型、行为和构建为准，未把Lint误记为通过 |

## 手机入口与身份检查

- P07的L1渲染回归通过：Web/PWA展示HarmonyOS安装步骤，并说明当前同一ChatGPT账号同步。
- `nativeMode`展示本机保存边界，在线入口固定为生产Site，含新窗口隔离属性；测试同时验证链接目标与属性。
- 仓库没有手机号输入、验证码发送或短信供应商配置。本版不收集手机号，也不把演示表单写成真实认证。
- 生产构建通过后，受管预览服务启动成功；本轮环境没有可用的浏览器控制能力，因此没有补报真实点击、390/320像素截图或PWA安装L3证据。v0.2.3已有的布局截图仍是历史基线，不能代替本版弹窗验收。

## 尚未通过或未执行

- 未使用第二个授权账号执行真实D1/R2跨设备同步；没有向生产伪造身份头。
- 未在HarmonyOS真机执行添加桌面、外链返回、麦克风、覆盖升级与数据保留；Android/iOS自RFC 0001起不再维护。
- 没有AGC应用配置、短信模板/配额、令牌验证后端、风控或公开登录入口授权，手机号登录未实现。
- 没有HarmonyOS SDK、签名材料和测试设备，未生成HAP。

源码发布、GitHub检查、线上Site部署与设备包是四类不同状态。提交后的GitHub与Site终态以各平台的提交、检查和部署记录为准；本报告不以“能访问URL”替代浏览器交互或真机证据。
