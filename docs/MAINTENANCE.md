...
4. **检查当前提交。** `Core validation / core-validation` 覆盖类型、内容、实际学习行为、Web构建和空库迁移。鸿蒙改动额外看共享资源结果；共享资源检查不代表HAP编译。Android/iOS工程和工作流已删除，不属于维护目标。
...
- `release-manifest.json` 是本次准备发布的源码版本声明；根、共享网页构建包与harmony版本和发布说明必须一致。Android/iOS旧版本只在Git历史中保留。
...
- HAP签名与真机状态独立记录；源码预发布不提升为“已可安装的鸿蒙版本”。

鸿蒙共享资源仍读取`mobile/`中的历史命名构建入口。Capacitor删除后，该入口只使用根目录锁定依赖；初始化与源码发布工作流不再安装第二套依赖。
