...
export function useStudy(owner:string|null){
 ...
 const syncNow=useCallback(async()=>{
  ...
  try{
   ...
   for(const op of journal(key).filter(op=>op.owner===owner&&!localStorage.getItem(key+':ack:'+op.id))){
    if(!valid())return;const r=await fetch('/api/study',{method:'POST',headers:{'Content-Type':'application/json','X-Lantern-Owner':owner},body:JSON.stringify(op.type==='profile'?{...op,data:readSnapshot(key).profile}:op)});if(!valid())return;if(!r.ok)throw Error('sync');localStorage.setItem(key+':ack:'+op.id,'1');
   }
 ...
 const enqueue=(type:Op['type'],value:Op['data'],d:StudyData):boolean=>{if(activeKey.current!==key)return false;const op:Op={id:crypto.randomUUID(),owner,type,data:value};try{localStorage.setItem(key+':op:'+op.id,JSON.stringify(op));if(type==='profile')localStorage.setItem(key+':profile',JSON.stringify(value));}catch{toast.error('本机存储不足，记录尚未保存');return false;}persist(d);setSync('pending');void syncNow();return true;};
